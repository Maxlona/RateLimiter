        ...
        public ConfigValues? BindConfig()
        {
            try
            {
                IConfigurationRoot config = new ConfigurationBuilder()
                    .AddJsonFile("rate-limiter-rules-dev.json", optional: false)
                    .Build();
                ...
                int? PerSeconds = int.Parse(config?.GetSection("RateLimiterRules:TimeLimiter:PerSecondsTimeFrame")?.Value);
                ...
                return new ConfigValues()
                {
                    Enabled = isEnabled,
                    MaxAllowed = MaxCalls,
                    PerSecondsTimeFrame = PerSeconds
                };
            }
            catch (Exception ex)
            {
